...
crossings_filtered <- crossings %>%
  filter(Crossing.Position=="At Grade", 
         ...
         Gate.Configuration != "",
         Number.Of.Bells > 0,
         Mast.Mounted.Flashing.Back.Lights == "Yes" | Mast.Mounted.Flashing.Side.Lights == "Yes",
         THR.Request.No == "") %>%
  select(Crossing.ID,
         Gate.Configuration,
         Gate.Configuration.Type,
         Number.Of.Bells,
         Cantilevered.Type.Structure,
         Mast.Mounted.Flashing.Light.Type,
         Mast.Mounted.Flashing.Back.Lights,
         Mast.Mounted.Flashing.Side.Lights,
         Other.Flashing.Lights.or.Warning.Devices.Type,
...
write.csv(crossings_gis2, "~/crossings_gates_bells_lights.csv", row.names = FALSE)